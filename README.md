...
| **6. GAN Variants & Image Generation** | Implementation and comparative analysis of **Naive GAN (DCGAN)**, **WGAN-GP**, and **BEGAN** architectures on the MNIST dataset. Evaluates training stability, Wasserstein distance with Gradient Penalty, and autoencoder-based discriminators, utilizing **FID scores** to quantitatively diagnose mode collapse. | `GAN` `WGAN-GP` `BEGAN` `FID` `Mode Collapse` |
